...
#
# These are shell helpers for browser tab client (bruvtab).
#

export BRUVTAB_CLIENT="$HOME/rc.arch/bz/bruvtab/bruvtab_client.py"
...
function fcl() {
    ...
    echo "$result" | while read -r line; do
        ...
        echo "$id"
    done | xargs bruvtab close
    #done | xargs $BRUVTAB_CLIENT close_tabs
}
...
function _activate_tab() {
    ...
    #$BRUVTAB_CLIENT activate_tab $strWindowTab
    bruvtab activate $strWindowTab
    _activate_browser $strWindowTab
}

function _close_tabs() {
    bruvtab close
    #$BRUVTAB_CLIENT close_tabs $*
}

function _list_tabs() {
    bruvtab list
    #$BRUVTAB_CLIENT list_tabs 1000
}
