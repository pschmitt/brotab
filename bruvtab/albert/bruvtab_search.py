...
To install put into:
    ~/.local/share/albert/org.albert.extension.python/modules/bruvtab_search.py
"""
...
from bruvtab.search.query import query as bruvtab_query

__title__ = "BruvTab Search"
__version__ = "0.1"
__triggers__ = "s "
__authors__ = ["Yuri Bochkarev"]
__dependencies__ = []
__exec_deps__ = ["bruvtab"]
...
def refresh_index():
    info('Bruvtab: refreshing index')
    subprocess.Popen(['bruvtab', 'index'])
...
def handleQuery(query):
    ...
    if tokens and tokens[0] == 'index':
        items.append(Item(
            ...
            subtext='> bruvtab index',
            actions=[
                Action(
                    id='reindex',
                    text='Reindex browser tabs',
                    callable=lambda: runDetachedProcess(cmdln=['bruvtab', 'index'], workdir='~'),
                )
            ]
        ))

    info('query %s' % user_query)
    query_results = bruvtab_query(
        SQL_DB_FILENAME, user_query, max_tokens=20, max_results=100, marker_cut='')
    info('bruvtab search: %s results' % len(query_results))
        ...
        items.append(Item(
            ...
            actions=[
                Action(
                    id='activate',
                    text='Activate',
                    callable=lambda: runDetachedProcess(cmdln=['bruvtab', 'activate', query_result.tab_id], workdir='~'),
                )
            ]
        ))
...
class Plugin(QueryHandler):
    ...
    def initialize(self): info('bruvtab initialize')
    def finalize(self): info('bruvtab finalize')
    def defaultTrigger(self): return __triggers__
    def handleQuery(self, query):
        info('bruvtab handleQuery')
